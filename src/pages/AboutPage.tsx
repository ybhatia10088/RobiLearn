import React from 'react'; import Layout from '@/components/layout/Layout'; import { motion } from 'framer-motion'; import { Heart, Target, Users, Lightbulb, Code, Cpu, Rocket, Star, Quote, ExternalLink, Mail, Github, Linkedin, BookOpen, Zap, Globe } from 'lucide-react';
...
const stats = [
{ number: '10,000+', label: 'Students Reached', icon:  },
{ number: '50+', label: 'Universities', icon:  },
{ number: '100+', label: 'Challenges', icon:  },
          ...
          {founders.map((founder, index) => (
            ...
            >
              <div className={`bg-dark-700/50 rounded-3xl p-8 border-2 border-${founder.color}-500/20 hover:border-${founder.color}-500/40 transition-all duration-500 group-hover:shadow-2xl backdrop-blur-sm h-full flex flex-col relative overflow-hidden`}>
                ...
                {/* Quote */}
                <div className={`bg-${founder.color}-500/5 rounded-2xl p-6 border-2 border-${founder.color}-500/20 relative z-10 group-hover:bg-${founder.color}-500/10 group-hover:border-${founder.color}-500/30 transition-all duration-300`}>
