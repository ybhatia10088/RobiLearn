...
const RobotModel: React.FC<RobotModelProps> = ({ robotConfig }) => {
  ...
  const RobotModel: React.FC<RobotModelProps> = ({ robotConfig }) => {
      // Add refs for new robot types
  const headRef = useRef<THREE.Group>(null);
  const leftArmRef = useRef<THREE.Group>(null);
  const rightArmRef = useRef<THREE.Group>(null);
  const leftLegRef = useRef<THREE.Group>(null);
  const rightLegRef = useRef<THREE.Group>(null);
  const spiderBodyRef = useRef<THREE.Group>(null);
  const spiderLegsRef = useRef<THREE.Group[]>([]);
  const tankBodyRef = useRef<THREE.Group>(null);
  const tankTurretRef = useRef<THREE.Group>(null);
  const tankBarrelRef = useRef<THREE.Group>(null);
  const tankTracksRef = useRef<THREE.Group[]>([]);

    // Enhanced Humanoid Robot with realistic articulation
  const HumanoidRobotGeometry = () => {
    const segments = 16; // Number of body segments for smooth curves
    
    return (
      <group>
        {/* Main body with articulated plating */}
        <group position={[0, 0.85, 0]}>
          {/* Torso core */}
          <mesh castShadow>
            <capsuleGeometry args={[0.18, 0.4, segments, segments]} />
            <meshStandardMaterial 
              color="#2c3e50" 
              metalness={0.7} 
              roughness={0.25}
          ...
          <mesh castShadow position={[0, 0.15, 0.2]}>
            <boxGeometry args={[0.25, 0.22, 0.1]} />
            <meshStandardMaterial 
              color="#3498db" 
              metalness={0.8} 
              roughness={0.2}
          ...
          {/* Abdominal joints */}
          <mesh castShadow position={[0, -0.15, 0.18]}>
            <cylinderGeometry args={[0.18, 0.18, 0.1, segments]} />
        ...
        <group ref={headRef} position={[0, 1.35, 0]}>
          ...
          {/* Optical sensors */}
          {[
            { pos: [-0.06, 0.05, 0.13], size: 0.04, color: "#3498db" },
            { pos: [0.06, 0.05, 0.13], size: 0.04, color: "#3498db" },
            { pos: [0, -0.05, 0.13], size: 0.03, color: "#e74c3c" }
          ].map((sensor, i) => (
            <mesh key={i} castShadow position={sensor.pos}>
              <sphereGeometry args={[sensor.size, 16, 16]} />
              <meshStandardMaterial 
                color={sensor.color}
                emissive={sensor.color}
          ...
          <mesh castShadow position={[0, -0.15, 0]}>
            <cylinderGeometry args={[0.08, 0.1, 0.1, segments]} />
            <meshStandardMaterial color="#1a2530" metalness={0.8} roughness={0.2} />
          </mesh>
        </group>
        
        {/* Arms with hydraulic systems */}
        ...
        <group ref={leftLegRef} position={[-0.15, 0.25, 0]}>
          <LegSegment side="left" />
        </group>
        <group ref={rightLegRef} position={[0.15, 0.25, 0]}>
          <LegSegment side="right" />
        </group>
      </group>
    );
  };

  // Reusable arm component
  const ArmSegment = ({ side }: { side: 'left' | 'right' }) => (
    <group rotation={[0, 0, side === 'left' ? 0.2 : -0.2]}>
      {/* Shoulder joint */}
      <mesh castShadow position={[0, 0.15, 0]}>
      ...
      <mesh castShadow position={[0, -0.25, 0]}>
        <capsuleGeometry args={[0.06, 0.4, 8]} />
        <meshStandardMaterial color="#3498db" metalness={0.7} roughness={0.25} />
      </mesh>
      ...
      <mesh castShadow position={[0, -0.8, 0]}>
        <capsuleGeometry args={[0.05, 0.3, 8]} />
        <meshStandardMaterial color="#3498db" metalness={0.7} roughness={0.25} />
      </mesh>
      ...
      <group position={[0, -1.15, 0]}>
        ...
        {/* Thumb */}
        <mesh castShadow position={[side === 'left' ? -0.07 : 0.07, -0.02, 0.06]}>
          <capsuleGeometry args={[0.02, 0.08, 6]} />
          <meshStandardMaterial color="#2c3e50" metalness={0.7} roughness={0.25} />
        </mesh>
        
        {/* Fingers */}
        {[-0.03, 0, 0.03].map((z, i) => (
          <mesh key={i} castShadow position={[0, -0.05, z]} rotation={[0.2, 0, 0]}>
            <capsuleGeometry args={[0.015, 0.1, 6]} />
            <meshStandardMaterial color="#2c3e50" metalness={0.7} roughness={0.25} />
          </mesh>
        ))}
      </group>
  ...
  // Reusable leg component
  const LegSegment = ({ side }: { side: 'left' | 'right' }) => (
    <group>
      {/* Hip joint */}
      <mesh castShadow position={[0, 0.15, 0]}>
      ...
      {/* Thigh */}
      <mesh castShadow position={[0, -0.25, 0]}>
        <capsuleGeometry args={[0.08, 0.5, 8]} />
        <meshStandardMaterial color="#3498db" metalness={0.7} roughness={0.25} />
      </mesh>
      ...
      {/* Shin */}
      <mesh castShadow position={[0, -1.05, 0]}>
        <capsuleGeometry args={[0.07, 0.6, 8]} />
        <meshStandardMaterial color="#3498db" metalness={0.7} roughness={0.25} />
      </mesh>
      ...
      <group position={[0, -1.6, 0.1]}>
        ...
        {/* Toes */}
        {[-0.06, 0, 0.06].map((x, i) => (
          <mesh key={i} castShadow position={[x, -0.04, 0.15]} rotation={[-0.1, 0, 0]}>
        ...
        {/* Heel */}
        <mesh castShadow position={[0, -0.04, -0.15]}>
  ...
  // Spider Walker with realistic multi-leg movement
  const SpiderWalkerGeometry = () => {
    const legPositions = [
      { x: -0.35, z: 0.3, rot: Math.PI/4 },
      { x: -0.45, z: 0.1, rot: Math.PI/6 },
      { x: -0.45, z: -0.1, rot: -Math.PI/6 },
      { x: -0.35, z: -0.3, rot: -Math.PI/4 },
      { x: 0.35, z: 0.3, rot: 3*Math.PI/4 },
      { x: 0.45, z: 0.1, rot: 5*Math.PI/6 },
      { x: 0.45, z: -0.1, rot: -5*Math.PI/6 },
      { x: 0.35, z: -0.3, rot: -3*Math.PI/4 }
    ];
    
    return (
      <group ref={spiderBodyRef}>
        {/* Main body with armored segments */}
        <mesh castShadow>
        ...
        {legPositions.map((pos, index) => (
          <group 
            key={index} 
            ref={el => spiderLegsRef.current[index] = el!}
  ...
  const SpiderLeg = () => (
    <group>
      ...
      {/* Femur */}
      <mesh castShadow position={[0, -0.2, 0.15]} rotation={[Math.PI/2, 0, 0]}>
        <cylinderGeometry args={[0.035, 0.035, 0.4, 8]} />
        <meshStandardMaterial color="#333333" metalness={0.8} roughness={0.2} />
      </mesh>
      
      {/* Tibia */}
      <mesh castShadow position={[0, -0.4, 0.15]} rotation={[Math.PI/2, 0, 0]}>
        <cylinderGeometry args={[0.03, 0.03, 0.35, 8]} />
        <meshStandardMaterial color="#2a2a2a" metalness={0.8} roughness={0.2} />
      </mesh>
      
      {/* Tarsus */}
      <mesh castShadow position={[0, -0.6, 0.15]} rotation={[Math.PI/2, 0, 0]}>
      ...
      <group position={[0, -0.75, 0.15]}>
        ...
        {/* Claws */}
        {[-0.03, 0.03].map((x, i) => (
          <mesh key={i} castShadow position={[x, -0.04, 0.02]}>
  ...
  // Heavy Mover Tank with realistic treads
  const HeavyTankGeometry = () => {
    const trackSegments = 24; // Number of track segments per side
    
    return (
      <group ref={tankBodyRef}>
        {/* Main hull */}
        <mesh castShadow>
        ...
        {/* Front slope */}
        <mesh castShadow position={[0, 0.1, 0.75]} rotation={[Math.PI/8, 0, 0]}>
          <boxGeometry args={[0.8, 0.4, 0.1]} />
          <meshStandardMaterial color="#666666" metalness={0.7} roughness={0.3} />
        </mesh>
        
        {/* Driver's viewport */}
        <mesh castShadow position={[0, 0.35, 0.65]}>
        ...
        {/* Main turret */}
        <group ref={tankTurretRef} position={[0, 0.7, 0]}>
          <mesh castShadow>
          ...
          {/* Main barrel */}
          <group ref={tankBarrelRef} position={[0, 0, 0.45]}>
            <mesh castShadow>
        ...
        <group position={[-0.45, -0.15, 0]}>
          {[...Array(trackSegments)].map((_, i) => {
            const angle = (i * Math.PI * 2) / trackSegments;
            return (
              <group 
                key={`left-${i}`}
                ref={el => tankTracksRef.current[i] = el!}
                position={[0, Math.cos(angle) * 0.35, Math.sin(angle) * 0.35]}
                rotation={[0, 0, angle]}
              >
                <TrackSegment />
              </group>
        ...
        <group position={[0.45, -0.15, 0]}>
          {[...Array(trackSegments)].map((_, i) => {
            ...
            return (
              <group 
                key={`right-${i}`}
                ref={el => tankTracksRef.current[i + trackSegments] = el!}
                position={[0, Math.cos(angle) * 0.35, Math.sin(angle) * 0.35]}
                rotation={[0, 0, angle]}
              >
                <TrackSegment />
        ...
        {[
          ...
          <mesh key={i} castShadow position={[pos.x, -0.15, pos.z]}>
        ...
        {/* Drive sprocket */}
        <mesh castShadow position={[-0.45, -0.15, -0.55]}>
  ...
  // Update animation loop for new robots
  useFrame((state, delta) => {
    // ... (existing animations)
    ...
    switch (robotConfig.type) {
      ...
      case 'humanoid':
        ...
        if (robotState?.isMoving) {
          ...
          const walkCycle = Math.sin(time * 3) * 0.2;
          const armSwing = Math.sin(time * 3) * 0.15;
          const headBob = Math.sin(time * 4) * 0.03;
          ...
          if (headRef.current) headRef.current.position.y = 0;
      ...
      case 'spider':
        // Realistic multi-leg walking animation
        if (spiderBodyRef.current && spiderLegsRef.current.length > 0) {
          const time = state.clock.getElapsedTime();
          const bodyBob = Math.sin(time * 4) * 0.02;
          spiderBodyRef.current.position.y = bodyBob;
          
          // Leg movement wave pattern
          spiderLegsRef.current.forEach((leg, index) => {
            if (!leg) return;
            
            const phaseOffset = (index % 4) * 0.5;
            const legLift = Math.sin(time * 4 + phaseOffset) * 0.3;
            
            // Only lift legs when moving
            if (robotState?.isMoving) {
              leg.rotation.x = legLift;
            } else {
              // Return to neutral position
      ...
      case 'tank':
        // Realistic track movement
        if (tankTracksRef.current.length > 0) {
          // Track rotation based on distance traveled
          const distance = group.current?.position.distanceTo(prevPosition.current) || 0;
          const trackRotation = distance / (Math.PI * 0.7); // Circumference of track
          
          tankTracksRef.current.forEach(segment => {
            if (segment) {
              segment.rotation.z += trackRotation;
            }
          });
          
          // Turret rotation
          if (tankTurretRef.current && moveCommands?.joint === 'turret') {
            const direction = moveCommands.direction === 'left' ? 1 : -1;
            tankTurretRef.current.rotation.y += direction * delta * 0.5;
          }
          
          // Barrel elevation
          if (tankBarrelRef.current && moveCommands?.joint === 'barrel') {
            const direction = moveCommands.direction === 'up' ? 1 : -1;
            tankBarrelRef.current.rotation.x = THREE.MathUtils.clamp(
              tankBarrelRef.current.rotation.x + direction * delta * 0.3,
              -Math.PI/6,
              Math.PI/12
            );
          }
  ...
  return (
    <group ref={group}>
      ...
      {robotConfig.type === 'spider' && <SpiderWalkerGeometry />}
      {robotConfig.type === 'tank' && <HeavyTankGeometry />}
    </group>
  );
};
